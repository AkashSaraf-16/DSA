    ...
    int largestRectangleArea(vector<int>& heights) {
        ...
        for(int i = 0 ; i < heights.size() ; i++){
            ...
            if(NSL.empty())
                ...
                indexL.push_back(NSL.top().second);     // else index of that number
            
            NSL.push({heights[i],i});       // pushing in stack current element and its index
        }
        for(int i = heights.size()-1 ; i >= 0 ; i--){
            ...
            if(NSR.empty())
                ...
                indexR.push_back(NSR.top().second);     // else index of that number
            
            NSR.push({heights[i],i});       // pushing in stack current element and its index
        }
